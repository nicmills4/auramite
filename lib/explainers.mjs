...
export function buildExplainers(scan) {
  ...
  if (scan.optOutMissingWhileSharing) {
    out.push({
      ...
      title: 'No working way for visitors to opt out',
      paragraph: `Your site shares data with advertisers, but offers no “Do Not Sell or Share My Personal Information” / “Your Privacy Choices” link — the opt-out method state laws require any business that sells or shares data to provide. Visitors have no way to say no.`,
      logLines: [{ text: 'checked homepage + footer links → no “Do Not Sell / Your Privacy Choices” link found' }],
      rule: `Missing required opt-out. Tractor Supply was fined $1.35M partly because its opt-out didn’t actually stop the sharing.`,
    });
  }
