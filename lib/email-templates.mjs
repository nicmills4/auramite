...
<tr><td style="padding:20px 28px 0;${FONT}font-size:12px;line-height:1.7;color:${FAINT};">
  Observations from an automated page load &mdash; not legal advice, and not a certification of compliance.
  Auramite is not a law firm. A scan sees one page load and can miss controls behind a consent dialog or
  policies published as PDFs; anything we flag as unconfirmed is worth checking by hand. Think we&rsquo;ve got
  something wrong? Reply and we&rsquo;ll re-scan.<br>
...
export function renderOrgReport({ pages, ranAt, baseUrl = 'https://auramite.io' }) {
  ...
  // ---- ONE structure; states change only the indicator color and values ---
  // Line 1: status word. Line 2: fixed field sentence. Line 3: fixed delta slot.
  const standing =
    scanned.length === 0 && failed.length > 0
      ? { word: "Couldn't check", bg: '#f5f5f4', fg: '#57534e' }
      : totalHigh > 0
        ? { word: 'Data leak detected', bg: '#fdecec', fg: RED }
        : anyGaps
          ? { word: 'Possible gaps', bg: '#fdf3e0', fg: AMBER }
          : { word: 'All clear', bg: '#e8f6ee', fg: GREEN };

  const deltaValue = baselineOnly
    ? 'first scan (baseline recorded)'
    : `${newCount > 0 ? `+${newCount} new` : 'nothing new'} · ${fixedCount > 0 ? `−${fixedCount} fixed` : 'nothing fixed'}`;

  const fieldLine = `${plural(totalHigh, 'high-severity finding')} open · ${scanned.length}/${pages.length} pages scanned`;

  const head = {
    ...standing,
    title: standing.word,
    body: fieldLine,
    delta: `Since last scan: ${deltaValue}`,
  };

  // Subject follows the same fixed pattern: status word, then nonzero deltas.
  const subject = `Auramite — ${standing.word}` +
  ...
  const bodyRows = [
    `<tr><td style="background:${head.bg};padding:18px 28px;${FONT}">
      ...
      <div style="font-size:12px;line-height:1.6;color:${head.fg};margin-top:2px;">${esc(head.delta)}</div>
    </td></tr>`,
  ...
  const L = [];
  L.push(`Your Auramite report — ${fmtWhen(ranAt)}`, '', head.title.toUpperCase(), head.body, head.delta, '');
  ...
  L.push('', '— Auramite · observations from an automated page load, not legal advice and not a');
  L.push('  certification of compliance. A scan sees one page load and can miss controls behind a');
  L.push('  consent dialog or policies published as PDFs. Think we got something wrong? Reply and');
  L.push('  we will re-scan.');
